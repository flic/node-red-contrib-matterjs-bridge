...
// @matter-server/ws-client (through at least 1.1.0) logs every incoming WS frame, every outgoing
// command and every event via console.debug with no off switch, which floods the Node-RED log
// (especially metered devices). By default we drop those per-frame/event/command debug lines
// ("WebSocket …" and "Incoming event"); the low-frequency lifecycle logs (Connected/Closed/Trying
// to connect) and all other console.debug output are preserved. Set MATTERJS_WS_DEBUG=1 to opt out
// and let everything through (debugging). Installed once per process.
(function silenceMatterWsDebug() {
    if (process.env.MATTERJS_WS_DEBUG === '1') { return; }
    ...
    console.debug = function (first) {
        if (typeof first === 'string' && (first.startsWith('WebSocket ') || first === 'Incoming event')) { return; }
...
function tapWebSocket(rawWs, emitter) {
    ...
    rawWs.addEventListener = function (type, listener, options) {
        ...
        // The whole per-attribute side channel hangs on the upstream client registering its
        // message handler through addEventListener. Record that it happened, so start() can
        // tell the difference between "quiet network" and "tap no longer catches anything".
        emitter._tapArmed = true;
        const wrapped = (event) => {
            try {
...
async function createBridge(url) {
    ...
    if (typeof client.addEventListener === 'function') {
        ...
        // Not all versions expose server_shutdown — registering for an unknown event either
        // no-ops or throws depending on version, so just try.
    ...
    bridge.start = async function () {
        ...
        try {
            ...
            // Self-check on the tap: if the upstream client connected without ever
            // registering a 'message' listener via addEventListener, its internals have
            // changed and the per-attribute side channel is dead — the connection would
            // look healthy while no attribute ever flows. Say so instead of going quiet.
            if (!bridge._tapArmed) {
                bridge.emit('error', Object.assign(
                    new Error('WS message tap never armed — @matter-server/ws-client may have changed '
                        + 'its socket handling; per-attribute events will not flow'),
                    { source: 'tap_check' }));
            }
...
// tapWebSocket/dispatchTapped exported for tests — they are the load-bearing coupling to
// the upstream client's internals, and the first thing to break on an upstream change.
module.exports = { createBridge, tapWebSocket, dispatchTapped };
